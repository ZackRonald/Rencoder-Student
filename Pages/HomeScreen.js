...
export default function HomeScreen({ navigation }) {
  ...
  const fetchTodayAttendance = async () => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.4.63:5000/getAttendance",
        { studEmail: email },
  ...
  const fetchCourses = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.4.63:5000/getCourse", {
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://192.168.4.63:5000/attendance",
        payload,
        {
          headers: {
